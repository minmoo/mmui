 ...
 * @param param0
 */
export const BottomSheetProvider = ({ children, options }: ProviderProps) => {
...
export const BottomSheetProvider2 = ({ children, options }: ProviderProps) => {
  ...
  const context: ContextState<unknown, unknown> = useMemo(
    () => ({
      ...
      show(id, props) {
        latestId.current = id;
        setOrders((orders) =>
          orders.filter((order) => order !== id).concat(id)
        );
        return new Promise((resolve) => {
          setLayers((layers) => ({
            ...layers,
  ...
  return (
    <LayerContext.Provider value={context}>
      {children}
      {orders.map((id) => {
        const { component: C, isOpen, props, resolve, options } = layers[
          id
        ] as Layer;
        if (resolve) {
          const onClose = (value: unknown) => {
